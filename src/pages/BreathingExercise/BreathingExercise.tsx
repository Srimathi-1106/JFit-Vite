...
const BreathingExercise = () => {
    ...
    if (isActive) {
      switch (phase) {
        case 'inhale':
          ...
          startTimer(customSettings.inhale, customSettings.hold1 > 0 ? 'hold1' : 'exhale');
          break;
        case 'hold1':
        ...
        case 'exhale':
          ...
          startTimer(customSettings.exhale, customSettings.hold2 > 0 ? 'hold2' : 'cycleComplete');
          break;
        case 'hold2':
  ...
  return (
    ...
    <div className="max-w-xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <h2 className="text-3xl font-bold text-center mb-6 bg-clip-text text-transparent bg-gradient-to-r from-[#200f7b] to-[#961aae]">
        Breathing Exercise
      </h2>
          ...
          {Object.keys(breathingPatterns).map((pattern) => (
            <button
              ...
              className={`py-2 px-4 rounded-md ${
                exerciseType === pattern ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-800'
              }`}
            >
      ...
      <div className="w-full mt-4">
        <div className="flex justify-between text-sm mb-1">
          <span>Progress: {Math.round((completedCycles / customSettings.cycles) * 100)}%</span>
          <span>{completedCycles}/{customSettings.cycles} cycles</span>
        </div>
        <div className="w-full bg-gray-200 rounded-full h-2.5">
          <div
            className="bg-blue-600 h-2.5 rounded-full"
            style={{ width: `${(completedCycles / customSettings.cycles) * 100}%` }}
          ></div>
        </div>
      </div>
